...
// 插入键值对
bool insert(HashMap *hashMap, const char *key, int value)
{
...
// 查找键值对
int search(HashMap *hashMap, const char *key)
{
    unsigned int index = hashFunction(key);
    do
    {
...
// 删除键值对
bool remove(HashMap *hashMap, const char *key)
{
    unsigned int index = hashFunction(key);
    do
    {
...
void testHashMap()
{
    ...
    // 插入键值对
    insert(hashMap, "one", 1);
    insert(hashMap, "two", 2);
    insert(hashMap, "three", 3);

    // 查找键值对
    printf("Value of 'one': %d\n", search(hashMap, "one"));
    printf("Value of 'two': %d\n", search(hashMap, "two"));
    printf("Value of 'three': %d\n", search(hashMap, "three"));

    // 删除键值对
    bool result = remove(hashMap, "two");
    if (result)
    {
